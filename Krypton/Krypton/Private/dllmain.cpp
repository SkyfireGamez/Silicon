...
#include "../../Krypton/Plugins/CrashReporter/Public/CrashReporter.h"
...
bool IsValidRadminIP()
{
    ...
    DWORD ret = GetAdaptersAddresses(AF_INET, GAA_FLAG_INCLUDE_PREFIX, nullptr, adapterAddresses, &bufferSize);
    if (ret == ERROR_BUFFER_OVERFLOW) {
        buffer.resize(bufferSize);
        adapterAddresses = reinterpret_cast<PIP_ADAPTER_ADDRESSES>(buffer.data());
        ret = GetAdaptersAddresses(AF_INET, GAA_FLAG_INCLUDE_PREFIX, nullptr, adapterAddresses, &bufferSize);
    }
...
void Main()
{
    ...
    {
        UKismetSystemLibrary::ExecuteConsoleCommand(UWorld::GetWorld(), FString(L"Fort.MME.Clambering 0"), nullptr);
        std::cout << " TEST2  TEST2  TEST2  TEST2  TEST2  TEST2  TEST2  TEST2  TEST2  TEST2  TEST2 \n TEST2  TEST2  TEST2  TEST2  TEST2  TEST2  TEST2  TEST2 \n TEST2  TEST2  TEST2 ";
    }
