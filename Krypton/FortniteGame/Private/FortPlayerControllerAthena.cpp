...
void AFortPlayerControllerAthena::ServerAcknowledgePossession(UObject* Context, FFrame& Stack)
{
	...
	else if (FConfiguration::bLateGame && (!FConfiguration::bKeepInventory || FConfiguration::bLateGame))
	{
		...
		PlayerController->WorldInventory->GiveItem(LateGame::GetAmmo(EAmmoType::EnergyCell), 20);
...
cheat fortmme - Fix Mantle
	...
	else
	{
		...
		else if (command == "fortmme")
		{
			...
			PlayerController->ClientMessage(FString(L"Applied FortMME fix"), FName(), 1.f);
		}
		...
		else if (command == "dumpitems" || command == "dump1")
		{

			std::stringstream ss;

			ss << "Generated by Silicon (https://github.com/SkyfireGamez/Silicon)\n";
			char version[6];
		...
		else if (command == "dumpplaylist" || command == "dump2")
		{
		...
		else if (command == "setmaxhealth" || command == "maxhealth" || command == "mh")
		{
			if (args.size() < 2)
			{
				PlayerController->ClientMessage(FString(L"Please choose an amount to set your health to!"), FName(), 1.f);
		...
		else if (command == "spawnbot" || command == "bot")
		{
			...
			for (int i = 0; i < Count; i++)
			{
				...
				PlayerBotID++;
				std::string Name = "Silicon Bot (#" + std::to_string(PlayerBotID) + ")";
