...
#include "../../Krypton/Public/Finders.h"
...
#include "../../Krypton/Public/LateGame.h"
#include "../Public/BuildingItemCollectorActor.h"
#include "../../Krypton/Public/GUI.h"
#include <random>
#include "../../Krypton/Public/Misc.h"
#include "../../Krypton/Public/Events.h"
#include "../Public/BattleRoyaleGamePhaseLogic.h"
#include "../Public/FortAthenaCreativePortal.h"
#include "../Public/FortPhysicsPawn.h"

#include <iostream>  // Make sure this is included somewhere
...
void AFortGameMode::ReadyToStartMatch_(UObject* Context, FFrame& Stack, bool* Ret)
{
    ...
    if (!GameMode->bWorldIsReady)
    {
        ...
        if (!FConfiguration::bUseCustomShit)
        {
        ...
        if (FConfiguration::bUseCustomShit)
        {
                BattleBusDef = FindObject<UObject>(FConfiguration::CustomBusDef);
                SupplyDropClass = FindObject<UClass>(FConfiguration::CustomSupplyDropDef);
        ...
        if constexpr (FConfiguration::WebhookURL && *FConfiguration::WebhookURL)
        {
            ...
            // P1
            auto payload = UEAllocatedString("{\"embeds\": [{\"title\": \"Server is joinable!\", \"fields\": [{\"name\":\"Version\",\"value\":\"") + version + "\"}, {\"name\":\"Playlist\",\"value\":\"" + (Playlist ? Playlist->PlaylistName.ToString() : "Playlist_DefaultSolo") + "\"}], \"color\": " + "\"7237230\", \"footer\": {\"text\":\"Silicon\", \"icon_url\":\"https://cdn.discordapp.com/attachments/1341168629378584698/1436803905119064105/L0WnFa.png.png?ex=6910ef69&is=690f9de9&hm=01a0888b46647959b38ee58df322048ab49e2a5a678e52d4502d9c5e3978d805&\"}, \"timestamp\":\"" + iso8601() + "\"}] }";
        ...
        sprintf_s(GUI::windowTitle, VersionInfo.EngineVersion >= 5.0 ? "Silicon (FN %.2f, UE %.1f): Joinable" : (VersionInfo.FortniteVersion >= 5.00 || VersionInfo.FortniteVersion < 1.2 ? "Silicon (FN %.2f, UE %.2f): Joinable" : "Silicon (FN %.1f, UE %.2f): Joinable"), VersionInfo.FortniteVersion, VersionInfo.EngineVersion);
...
bool AFortGameMode::StartAircraftPhase(AFortGameMode* GameMode, char a2)
{
    ...
    if constexpr (FConfiguration::WebhookURL && *FConfiguration::WebhookURL)
    {
        ...
        // P2
        auto payload = UEAllocatedString("{\"embeds\": [{\"title\": \"Match has started!\", \"fields\": [{\"name\":\"Version\",\"value\":\"") + version + "\"}, {\"name\":\"Playlist\",\"value\":\"" + (Playlist ? Playlist->PlaylistName.ToString() : "Playlist_DefaultSolo") + "\"},{\"name\":\"Players\",\"value\":\"" + std::to_string(GameMode->AlivePlayers.Num()).c_str() + "\"}], \"color\": " + "\"7237230\", \"footer\": {\"text\":\"Silicon\", \"icon_url\":\"https://cdn.discordapp.com/attachments/1341168629378584698/1436803905119064105/L0WnFa.png.png?ex=6910ef69&is=690f9de9&hm=01a0888b46647959b38ee58df322048ab49e2a5a678e52d4502d9c5e3978d805&\"}, \"timestamp\":\"" + iso8601() + "\"}] }";
    ...
    sprintf_s(GUI::windowTitle, VersionInfo.EngineVersion >= 5.0 ? "Silicon (FN %.2f, UE %.1f): Match started" : (VersionInfo.FortniteVersion >= 5.00 || VersionInfo.FortniteVersion < 1.2 ? "Silicon (FN %.2f, UE %.2f): Match started" : "Silicon (FN %.1f, UE %.2f): Match started"), VersionInfo.FortniteVersion, VersionInfo.EngineVersion);
    SetConsoleTitleA(GUI::windowTitle);
