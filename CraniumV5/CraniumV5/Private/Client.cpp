...
// TODO:  move more annoying shit to config2
...
void ClientThread()
{
	...
	while (true)
	{
		...
		if (!FConfiguration::bClientConsole67K)
		{
			...
			else if (!GetAsyncKeyState(VK_F3) && !GetAsyncKeyState(VK_F4) && !GetAsyncKeyState(VK_F2))
				bPressed = false;
	...
	if (VersionInfo.FortniteVersion >= 10 || FConfiguration::bForceRespawns)
	{
		...
		for (int i = 0; i < TUObjectArray::Num(); i++)
		{
			...
			if (Object && Object->IsA((UClass*)PlaylistClass))
			{
				...
				if (FConfiguration::bMagicMethodCreative)
				{
					auto Name = Object->Name.ToString();
